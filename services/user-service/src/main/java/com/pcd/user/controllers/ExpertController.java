...
public class ExpertController {
    ...
    public ResponseEntity<Map<String, Object>> generateReport(
        ...
        User user = (User) authentication.getPrincipal();
        logger.info("Report generation requested by expert {}: Analysis ID: {}",
                user.getEmail(), analysisId);

        try {
            // In a real implementation, you would:
            // 1. Validate the analysis ID
            // 2. Call the Report service to generate a report
            ...
            response.put("generatedBy", user.getUsername());
    ...
    public ResponseEntity<Map<String, Object>> annotateImage(
            @Valid @RequestBody AnnotationRequest request,
            Authentication authentication) {

        User user = (User) authentication.getPrincipal();
        logger.info("Image annotation submitted by expert {}: Image ID: {}",
                user.getEmail(), request.getImageId());

        try {
            // In a real implementation, you would:
            // 1. Validate the annotation data
            // 2. Store the annotations
            // 3. Update the image metadata

            String annotationId = "ANT-" + System.currentTimeMillis();

            Map<String, Object> response = new HashMap<>();
            response.put("status", "success");
            response.put("message", "Annotations saved successfully");
            response.put("annotationId", annotationId);
            response.put("imageId", request.getImageId());
            response.put("createdAt", LocalDateTime.now());
            response.put("createdBy", user.getUsername());
            response.put("annotations", request.getAnnotations());

            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("Error saving annotations: {}", e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Error saving annotations");
        }
    }
    ...
    public static class AnnotationRequest {
        ...
        @NotNull(message = "Annotations are required")
